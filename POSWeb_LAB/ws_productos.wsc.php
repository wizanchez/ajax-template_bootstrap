...
define(URL_WEB_ws,DF_URL_WSDL_RAIZ);
#define(URL_WEB_ws,'http://191.168.0.57/emssanar/');
			...
			##	WEB SERVICE EN TABLAS ORIGINALES
				define(URL_WEB_SERVICE_SERVIDOR,URL_WEB_ws.'ws_asistencial_v20.wsdl.php');
			#############################################################################################################
...
class ws_productos{
	...
	public function verificarError($mess='')
	{
		global $cliente;
	...
	public function conectarWebService($url='', $tipo='',$mess='')
	{
		global $cliente;
		...
		if($cadena){
			...
			## actualizamos los comvenios	
				for($p=0;$p<count($a_productos[0]['bodega'][0]['convenios']);++$p){
						...
						#----------------------------------------------------------------------------------------
						#	averiguo la informacion
								$a_vector 	=	ws_productos::pos_items_info($value,$tipo);
						#----------------------------------------------------------------------------------------

				$a_vec['shortdescription']	=$a_vector[0]['shortdescription'];
				$a_vec['description']		=$a_vector[0]['description'];
				$a_vec['item_name']			=$a_vector[0]['item_name'];
				$a_vec['quantity']			=$a_vector[0]['quantity'];
				$a_vec['buy_price']			=$a_vector[0]['buy_price'];
				$a_vec['unit_price']		=$a_vector[0]['unit_price'];
				$a_vec['total_cost']		=$a_vector[0]['total_cost'];
				$a_vec['tax_percent']		=$a_vector[0]['tax_percent'];
