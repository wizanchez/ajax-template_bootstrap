...
if($form == "$lang->allCustomersReport") {
	...
	if(isset($_GET['customer_search'])) {
		$search = $_GET['customer_search'];
		$customer_result = mysql_query("SELECT first_name,last_name,id,account_number FROM $customers_table WHERE last_name like \"%$search%\" or first_name like \"%$search%\" OR account_number = '$search' ORDER by last_name",$dbf->conn);
	}
	...
	echo "<center><form name='search' action='form.php' method='GET'>
	$lang->findCustomer: <input type='text' size='8' name='customer_search'>
	<input type='hidden' name='report' value='$lang->pacienteReport' value='Go'>
	<input type='submit' value='Go'>
	</form>";
	if(count($customer_result) > 0) {
	...
	$f1->endForm();
} elseif($form == "$lang->profitReportDispen") {
	$option_values = array("$today","$yesterday","$week","$thismonth","$lastmonth","$thisyear","$alltime");
	$option_titles = array("$lang->today","$lang->yesterday","$lang->last7days","$lang->thisMonth","$lang->lastMonth","$lang->thisYear","$lang->allTime");
	$f1 = new form('profitDispen.php','POST','profit','200',$cfg_theme,$lang);
	$f1->createSelectField("<b>$lang->dateRange</b>",'date_range',$option_values,$option_titles,'200');
	$f1->endForm();
} elseif($form == "$lang->copyRango"){
	$option_values = array(1,2,3);
	$option_titles = array("Venta","Dispensaci&oacute;n","Evento");
    echo '<br><br><br>';
	$f1 = new form('../sales/printcopysalerango.php','POST','print_sales','450',$cfg_theme,$lang);
	$option_values2 = array("$today","$yesterday","$week","$thismonth","$lastmonth","$thisyear","$alltime");
	$option_titles2 = array("$lang->today","$lang->yesterday","$lang->last7days","$lang->thisMonth","$lang->lastMonth","$lang->thisYear","$lang->allTime");
	$f1->createSelectField("<b>$lang->selecttipoVta</b>",'selected_tipovta',$option_values,$option_titles,'150');
	echo"
		<tr bgcolor=$f1->row_color>
		  <td width='$td_width' colspan=2 align='center'><font color='$f1->text_color'><strong>RANGO DE FECHAS</strong></font></td>
		</tr>\n";
	echo"
		<tr bgcolor=$f1->row_color>
		  <td width='$td_width'><font color='$f1->text_color'><strong>DESDE</strong></font></td>
		  <td><input type='text' name='desde' id='desde' value='".date('Y-m-d')."' size='12'><input type='button' id='f_btn1' value='...'/></td>
		</tr>\n";
	echo"
		<tr bgcolor=$f1->row_color>
		  <td width='$td_width'><font color='$f1->text_color'><strong>HASTA</strong></font></td>
		  <td><input type='text' name='hasta' id='hasta' value='".date('Y-m-d')."' size='12'><input type='button' id='f_btn2' value='...'/></td>
		</tr>\n";        
...
$dbf->closeDBlink(); ?>
    <script src="../js/calendar/js/jscal2.js"></script>
    <script src="../js/calendar/js/lang/es.js"></script>
    ...
    <link rel="stylesheet" type="text/css" href="../js/calendar/css/steel/steel.css" />   
