...
if($amount[$m] > 0){
    ...
    if(mysql_numrows($result) == 0){
            $field_paywith = array('sale_id', 'paymethod_id', 'amount', 'doc_number',
                ...
                'paymethoddetail_id');
            $field_datapay = array($sale_id, $paymethodid[$m], $amount[$m], $doc_num[$m],
                $banco, $auth_num[$m], 0, date("Y-m-d"), $_SESSION['session_user_id'], $entity[$m]);
...
?>       
        ...
        <span ><strong><?=strtoupper($lang->date)?>: </strong><?=date("Y-m-d")?></span><span style="float: right;"><strong><?=strtoupper($lang->hoursec)?>: </strong><?=date('h:m:s')?></span>
...
<?php
//ME TRAIGO TODOS LOS PRODUCTOS DE LA FACTURA CABECERA QUE SEAN DE TIPO VTA
...
?>
    <tr>
        ...
        <td nowrap nowrap style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: left;"><?=$itemdesc?></td>
...
<?php
    $result = mysql_query("SELECT * FROM $paywith_table WHERE sale_id = '$sale_id'",$dbf->conn);
    while ($row = mysql_fetch_assoc($result)){
