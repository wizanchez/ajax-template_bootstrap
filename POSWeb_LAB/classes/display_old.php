...
class display
{
	...
	function displayReportTableDispe($tableprefix,$tablename,$tableheaders,$tablefields,$wherefield,$wheredata,$date1,$date2,$orderby,$subtitle)
	{
		...
		if($wherefield!='' and $wheredata!='' and $date1=='' and $date2=='')
		{
			...
			$result = mysql_query("SELECT * FROM $table WHERE $wherefield = \"$wheredata\" AND tipo_vta = 2 ORDER BY $orderby",$this->conn);
		}
		elseif($wherefield!='' and $wheredata!='' and $date1!='' and $date2!='')
		{
			$result = mysql_query("SELECT * FROM $table WHERE $wherefield = \"$wheredata\" AND tipo_vta = 2 and date between \"$date1\" and \"$date2\" ORDER BY $orderby",$this->conn);
		}
		elseif($date1!='' and $date2!='')
		{
			$result = mysql_query("SELECT * FROM $table WHERE  date between \"$date1\" and \"$date2\"  AND tipo_vta = 2 ORDER BY $orderby",$this->conn);

		}
		else
		{
			$result = mysql_query("SELECT * FROM $table WHERE tipo_vta = 2 ORDER BY $orderby",$this->conn);
		}
		echo '<hr>';
		if(@mysql_num_rows($result) ==0)
		{
