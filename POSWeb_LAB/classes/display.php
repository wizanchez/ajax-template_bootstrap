<?php class display {
	var $conn;
	var $lang;
	var $title_color,$list_of_color,$table_bgcolor,$cellspacing,$cellpadding,$border_style,$border_width,$border_color,$header_rowcolor,$header_text_color,$headerfont_face,$headerfont_size,$rowcolor1,$rowcolor2,$rowcolor_text,$rowfont_face,$rowcolor_link,$rowfont_size,$sale_bg;
	...
	function idToFieldWh($tablename,$field,$field2,$id) {
		...
		$result = mysql_query("SELECT $field FROM $tablename WHERE $field2=\"$id\"",$this->conn);
		$row = mysql_fetch_assoc($result);
		return $row[$field];
	}
	...
	function displayProductosHora($tableprefix,$tablename,$tableheaders,$tablefields,$wherefield,$wheredata,$date1,$date2,$orderby,$subtitle) {
		echo "<center><h4><font color='$this->list_of_color'>$subtitle</font></h4></center>";
		$tablewidth = '85%';
		$table = "$tableprefix"."$tablename";
		echo "\n".'<center>';
		$result = mysql_query("SELECT pos_sales.date AS date,
							  		CONCAT(pos_users.first_name,' ',pos_users.last_name) AS operario,  
									SUM(pos_sales.sale_total_cost) AS venta
								FROM pos_sales JOIN pos_sales_items ON pos_sales.id=pos_sales_items.sale_id 
								WHERE  pos_sales.status=1 AND pos_sales.date='".$wheredata."' 
		...
		while($row = mysql_fetch_assoc($result)) {
			...
			$venta=number_format($row['venta'],2,',','.');
			echo "\n<td  align='center'>\n<font color='$this->rowcolor_text' face='$this->rowfont_face' size='$this->rowfont_size'>$fecha</font>\n</td>";
			echo "<td  align='center'>\n<font color='$this->rowcolor_text' face='$this->rowfont_face' size='$this->rowfont_size'>$operario</font>\n</td>";
			echo "<td  align='center'>\n<font color='$this->rowcolor_text' face='$this->rowfont_face' size='$this->rowfont_size'>$venta</font>\n</td>\n";
		}
		echo '</table>'."\n";
	}
	function displayReportTableVentaDiariaOperador($tableprefix,$tablename,$tableheaders,$tablefields,$wherefield,$wheredata,$date1,$date2,$orderby,$subtitle) {
		echo "<center><h4><font color='$this->list_of_color'>$subtitle</font></h4></center>";
		$tablewidth = '85%';
		$table = "$tableprefix"."$tablename";
		echo "\n".'<center>';
		$result = mysql_query("SELECT pos_sales.date AS date,
							  		CONCAT(pos_users.first_name,' ',pos_users.last_name) AS operario,  
									SUM(pos_sales.sale_total_cost) AS venta
								FROM pos_sales JOIN pos_users ON pos_sales.sold_by=pos_users.id 
								WHERE  pos_sales.status=1 AND pos_sales.date='".$wheredata."' 
								GROUP BY pos_sales.sold_by",
								$this->conn);
		/*echo "SELECT pos_sales.date AS date,
							  		CONCAT(pos_users.first_name,' ',pos_users.last_name) AS operario,  
									SUM(pos_sales.sale_total_cost) AS venta
								FROM pos_sales JOIN pos_users ON pos_sales.sold_by=pos_users.id 
								WHERE  pos_sales.status=1 AND pos_sales.date='".$wheredata."' 
	...
	function displayReportTable($tableprefix,$tablename,$tableheaders,$tablefields,$wherefield,$wheredata,$date1,$date2,$orderby,$subtitle) {
		...
		if($tablename == 'sales_items') {
			...
			if($wherefield != '' and $wheredata != '' and $date1 == '' and $date2 == '') {
				$result = mysql_query("SELECT * FROM $table WHERE tipo_vta = 1 AND $wherefield = \"$wheredata\" ORDER BY $orderby",$this->conn);
			} elseif($wherefield != '' and $wheredata != '' and $date1 != '' and $date2 != '') {
				$result = mysql_query("SELECT * FROM $table WHERE tipo_vta = 1 AND $wherefield = \"$wheredata\" and date between \"$date1\" and \"$date2\" ORDER BY $orderby",$this->conn);
			} elseif($date1 != '' and $date2 != '') {
				$result = mysql_query("SELECT * FROM $table WHERE tipo_vta = 1 AND date between \"$date1\" and \"$date2\" ORDER BY $orderby",$this->conn);
			} else {
				$result = mysql_query("SELECT * FROM $table WHERE tipo_vta = 1 ORDER BY $orderby",$this->conn);
			}
		}
		echo '<hr>';
		if(@mysql_num_rows($result) == 0) {
	...
	function displayReportTableDispe($tableprefix,$tablename,$tableheaders,$tablefields,$wherefield,$wheredata,$date1,$date2,$orderby,$subtitle) {
		echo "<center><h4><font color='$this->list_of_color'>$subtitle</font></h4></center>";
		$tablewidth = '85%';
		$table = "$tableprefix"."$tablename";
		...
		if($tablename == 'sales_items') {
			if($wherefield != '' and $wheredata != '' and $date1 == '' and $date2 == '') {
                ...
                FROM $table,pos_sales 
				WHERE pos_sales.id = $sales_items_table.sale_id AND pos_sales.status = 1 and pos_sales.tipo_vta = 2 AND $sales_items_table.tipo = 2 
				AND $sales_items_table.qtyrecetada > 0  
				AND $table.$wherefield = \"$wheredata\"   
				ORDER BY pos_sales.$orderby",$this->conn);
				//echo "SELECT * FROM $table WHERE  $wherefield = \"$wheredata\"  ORDER BY $orderby";
			} elseif($wherefield != '' and $wheredata != '' and $date1 != '' and $date2 != '') {
				$result = mysql_query("SELECT $table.* 
				FROM $table,pos_sales 
				WHERE pos_sales.id = $sales_items_table.sale_id AND pos_sales.status = 1 and pos_sales.tipo_vta = 2 AND $sales_items_table.tipo = 2 
				AND $sales_items_table.qtyrecetada > 0  
				AND $table.$wherefield = \"$wheredata\"  and pos_sales.date between \"$date1\" and \"$date2\"  
				ORDER BY pos_sales.$orderby",$this->conn);
				//echo "SELECT * FROM $table WHERE $wherefield = \"$wheredata\"  and date between \"$date1\" and \"$date2\" ORDER BY $orderby";
			} elseif($date1 != '' and $date2 != '') {
				$result = mysql_query("SELECT $table.* 
				FROM $table,pos_sales 
				WHERE pos_sales.id = $sales_items_table.sale_id AND pos_sales.status = 1 and pos_sales.tipo_vta = 2 AND $sales_items_table.tipo = 2 
				AND $sales_items_table.qtyrecetada > 0  
				AND pos_sales.date between \"$date1\" and \"$date2\"  
				ORDER BY pos_sales.$orderby",$this->conn);
				//echo "SELECT * FROM $table WHERE  date between \"$date1\" and \"$date2\"   ORDER BY $orderby";
			} else {
				$result = mysql_query("SELECT $table.* 
				FROM $table,pos_sales 
				WHERE pos_sales.id = $sales_items_table.sale_id AND pos_sales.status = 1 and pos_sales.tipo_vta = 2 AND $sales_items_table.tipo = 2 
				AND $sales_items_table.qtyrecetada > 0  
				ORDER BY pos_sales.$orderby",$this->conn);
				//echo "SELECT * FROM $table  ORDER BY $orderby";
			}
		} else {
			if($wherefield != '' and $wheredata != '' and $date1 == '' and $date2 == '') {
				$result = mysql_query("SELECT $table.id, $table.date, $table.customer_id as paciente_id, $table.sale_sub_total, $table.sale_total_cost, $table.paid_with, $table.items_purchased, $table.sold_by, $table.comment, $table.changee, $table.invoicenumber, $table.status, $table.descuentporc, $table.value_discount, $table.tipo_vta,$table.formula  
	...
	function displaySaleManagerTable($tableprefix,$where1,$where2) {
		...
		if($where1 != '' and $where2 != '') {
			$sale_query = "SELECT * FROM $sales_table WHERE status = 1 and tipo_vta = 1 and invoicenumber between \"$where1\" and \"$where2\" ORDER BY id DESC";
	...
	function displayEventoManagerTable($tableprefix,$where1,$where2) {
		...
		$sales_items_table = "$tableprefix"."sales_items";
		if($where1 != '' and $where2 != '') {
			$sale_query = "SELECT $sales_table.id, $sales_table.date, $sales_table.customer_id as paciente_id, $sales_table.sale_sub_total, $sales_table.sale_total_cost, $sales_table.paid_with, $sales_table.items_purchased, $sales_table.sold_by, $sales_table.comment, $sales_table.changee, $sales_table.invoicenumber, $sales_table.status, $sales_table.descuentporc, $sales_table.value_discount, $sales_table.tipo_vta  FROM $sales_table,$sales_items_table WHERE $sales_table.id = $sales_items_table.sale_id AND  $sales_table.status = 1 AND $sales_table.tipo_vta = 2 AND $sales_items_table.tipo = 3 AND $sales_items_table.qtyrecetada > 0 AND $sales_items_table.quantity_purchased > 0  and $sales_table.invoicenumber between \"$where1\" and \"$where2\" GROUP BY $sales_table.id ORDER BY $sales_table.id DESC";
			$sale_result = mysql_query($sale_query,$this->conn);
		} else {
			$sale_query = "SELECT $sales_table.id, $sales_table.date, $sales_table.customer_id as paciente_id, $sales_table.sale_sub_total, $sales_table.sale_total_cost, $sales_table.paid_with, $sales_table.items_purchased, $sales_table.sold_by, $sales_table.comment, $sales_table.changee, $sales_table.invoicenumber, $sales_table.status, $sales_table.descuentporc, $sales_table.value_discount, $sales_table.tipo_vta  FROM $sales_table,$sales_items_table WHERE $sales_table.id = $sales_items_table.sale_id AND  $sales_table.status = 1 AND $sales_table.tipo_vta = 2 AND $sales_items_table.tipo = 3 AND $sales_items_table.qtyrecetada > 0 AND $sales_items_table.quantity_purchased > 0 GROUP BY $sales_table.id ORDER BY $sales_table.id DESC";
			$sale_result = mysql_query($sale_query,$this->conn);
		}
		$sales_tableheaders = array("{$this->lang->date}","{$this->lang->customerPac}","{$this->lang->itemsPurchased}","{$this->lang->paidWith}","{$this->lang->soldBy}","{$this->lang->saleSubTotal}","{$this->lang->saleTotalCost}","{$this->lang->saleComment}");
		$sales_tablefields = array('date','paciente_id','items_purchased','paid_with','sold_by','sale_sub_total','sale_total_cost','comment');
		$sales_items_tableheaders = array("{$this->lang->itemName}","{$this->lang->brand}","{$this->lang->category}","{$this->lang->supplier}","{$this->lang->quantityFormula}","{$this->lang->quantityDispensa}","{$this->lang->unitPrice}","{$this->lang->tax}","{$this->lang->itemTotalCost}","{$this->lang->updateItem}","{$this->lang->deleteItem}");
		$sales_items_tablefields = array('item_id','brand_id','category_id','supplier_id','qtyrecetada','quantity_purchased','item_unit_price','item_total_tax','item_total_cost');
		if(@mysql_num_rows($sale_result) < 1) {
			echo "<div align='center'>You do not have any data in the <b>sales</b> tables.</div>";
			exit();
		}
	...
	function displayTotalsReport($tableprefix,$total_type,$tableheaders,$date1,$date2,$where1,$where2) {
		...
		if($total_type == 'customers') {
			...
			while($row = mysql_fetch_assoc($item_result)) {
				...
				$query2 = mysql_query("SELECT * FROM $sales_table WHERE status = 1 and tipo_vta=1 AND date between \"$date1\" and \"$date2\" ORDER by id ASC",$this->conn);
				$sale_row1 = mysql_fetch_assoc($query2);
				$low_sale_id = $sale_row1['id'];
				$query3 = mysql_query("SELECT * FROM $sales_table WHERE status = 1 and tipo_vta=1 AND date between \"$date1\" and \"$date2\" ORDER by id DESC",$this->conn);
			...
			$temp_supplier = $this->idToField($suppliers_table,'supplier',$this->idToField($items_table,'supplier_id',$temp_item_id));
			$item_name = $this->formatData('item_id',$temp_item_id,$tableprefix);
			$query2 = mysql_query("SELECT * FROM $sales_table WHERE tipo_vta=1 AND date between \"$date1\" and \"$date2\" ORDER by id ASC",$this->conn);
			$sale_row1 = mysql_fetch_assoc($query2);
			$low_sale_id = $sale_row1['id'];
			$query3 = mysql_query("SELECT * FROM $sales_table WHERE tipo_vta=1 AND date between \"$date1\" and \"$date2\" ORDER by id DESC",$this->conn);
			$sale_row2 = mysql_fetch_assoc($query3);
			$high_sale_id = $sale_row2['id'];
			$query4 = "SELECT * FROM $sales_items_table WHERE item_id=\"$temp_item_id\" and sale_id between \"$low_sale_id\" and \"$high_sale_id\"";
			$result4 = mysql_query($query4,$this->conn);
			...
			while($row = mysql_fetch_assoc($result)) {
				...
				$result2 = mysql_query("SELECT * FROM $sales_table WHERE status = 1 and tipo_vta=1 AND date=\"$distinct_date\"",$this->conn);
				echo "\n<tr bgcolor=$this->rowcolor1>\n";
			...
			$result = mysql_query($query);
			while($row = mysql_fetch_assoc($result)) {
				$result2 = mysql_query("SELECT supplier FROM pos_suppliers WHERE id = ".$row['supplier_id']." ",$this->conn);
				...
				echo "\n<tr bgcolor=$this->rowcolor1>\n";
				echo "<td align='center'>\n<font color='$this->rowcolor_text' face='$this->rowfont_face' size='$this->rowfont_size'>".$row['item_number']."</font>\n</td>";
				echo "<td align='center'>\n<font color='$this->rowcolor_text' face='$this->rowfont_face' size='$this->rowfont_size'>".$row['item_name']."</font>\n</td>";
				echo "<td align='center'>\n<font color='$this->rowcolor_text' face='$this->rowfont_face' size='$this->rowfont_size'>".$row_2['supplier']."</font>\n</td>";
				echo "<td align='center'>\n<font color='$this->rowcolor_text' face='$this->rowfont_face' size='$this->rowfont_size'>".number_format($row['buy_price'],2)."</font>\n</td>";
				echo "<td align='center'>\n<font color='$this->rowcolor_text' face='$this->rowfont_face' size='$this->rowfont_size'>".number_format($row['unit_price'],2)."</font>\n</td>";
				echo "<td align='center'>\n<font color='$this->rowcolor_text' face='$this->rowfont_face' size='$this->rowfont_size'>".number_format($margen,2)."</font>\n</td>";
				echo "<td align='center'>\n<font color='$this->rowcolor_text' face='$this->rowfont_face' size='$this->rowfont_size'>".number_format($row['quantity'])."</font>\n</td>";
				echo "<td align='center'>\n<font color='$this->rowcolor_text' face='$this->rowfont_face' size='$this->rowfont_size'>".number_format($tot_a_costo,2)."</font>\n</td>";
				echo "<td align='center'>\n<font color='$this->rowcolor_text' face='$this->rowfont_face' size='$this->rowfont_size'>".number_format($tot_a_veta,2)."</font>\n</td>";
				echo "</tr>";
    ...
    <td align='center'>\n<font color='$this->rowcolor_text' face='$this->rowfont_face' size='$this->rowfont_size'>".number_format($tot_costo,2)."</td>
    <td align='center'>\n<font color='$this->rowcolor_text' face='$this->rowfont_face' size='$this->rowfont_size'>".number_format($tot_pvp,2)."</td>
    <td align='center'>\n<font color='$this->rowcolor_text' face='$this->rowfont_face' size='$this->rowfont_size'>".number_format($tot_margen,2)."</td>
    <td align='center'>\n<font color='$this->rowcolor_text' face='$this->rowfont_face' size='$this->rowfont_size'>".number_format($tot_quantity)."</td>
    <td align='center'>\n<font color='$this->rowcolor_text' face='$this->rowfont_face' size='$this->rowfont_size'>".number_format($total_a_costo,2)."</td>
    <td align='center'>\n<font color='$this->rowcolor_text' face='$this->rowfont_face' size='$this->rowfont_size'>".number_format($total_a_vta,2)."</td>
  </tr>";
			echo '</table>';
		}
	}
	////----------////
	function displayDispeTotalsReport($tableprefix,$total_type,$tableheaders,$date1,$date2,$where1,$where2) {
		$sales_table = "$tableprefix".'sales';
		$sales_items_table = "$tableprefix".'sales_items';
		$items_table = "$tableprefix".'items';
		$brands_table = "$tableprefix".'brands';
		...
		if($total_type == 'pos_pacientes') {
			...
			$query = "SELECT $customer_table.first_name, $customer_table.last_name, $customer_table.account_number, $customer_table.phone_number, $customer_table.email, $customer_table.street_address, $customer_table.comments, $customer_table.id  FROM $customer_table,$sales_table,$sales_items_table 
				...
				AND $sales_items_table.qtyrecetada > 0 AND $sales_items_table.quantity_purchased > 0 AND $customer_table.id = $sales_table.customer_id and $sales_table.date between \"$date1\" and \"$date2\" GROUP BY $sales_table.customer_id  ORDER BY $customer_table.last_name";
			$customer_result = mysql_query($query,$this->conn);
			$temp_cust_id = 0;
			$accum_sub_total = 0;
			$accum_total_cost = 0;
			...
			while($row = mysql_fetch_assoc($customer_result)) {
				...
				WHERE $sales_table.id = $sales_items_table.sale_id AND $sales_table.tipo_vta = 2 AND $sales_items_table.tipo = 2 AND $sales_items_table.qtyrecetada > 0 AND $sales_items_table.quantity_purchased > 0 AND $sales_table.customer_id=\"$temp_cust_id\" and $sales_table.date between \"$date1\" and \"$date2\"	GROUP BY $sales_table.id ORDER BY $sales_table.id DESC";
			...
			echo '</tr>'."\n\n";
			$query = "SELECT $items_table.* FROM $items_table,$sales_items_table,$sales_table WHERE 
            $items_table.id = $sales_items_table.item_id and 
            $sales_table.id = $sales_items_table.sale_id AND $sales_table.tipo_vta = 2 AND $sales_items_table.tipo = 2 AND $sales_items_table.qtyrecetada > 0 
				AND $sales_table.date between \"$date1\" and \"$date2\"
				AND $sales_items_table.quantity_purchased > 0 GROUP BY $items_table.id ORDER BY item_name";
			$item_result = mysql_query($query,$this->conn);
			$temp_item_id = 0;
			$accum_sub_total = 0;
			$accum_total_cost = 0;
			...
			while($row = mysql_fetch_assoc($item_result)) {
				...
				AND $sales_table.date between \"$date1\" and \"$date2\"
				AND $sales_items_table.quantity_purchased > 0 GROUP BY $sales_table.id ORDER BY $sales_table.id ASC",$this->conn);
				...
				WHERE $sales_table.id = $sales_items_table.sale_id AND $sales_table.tipo_vta = 2 AND $sales_items_table.tipo = 2 AND $sales_items_table.qtyrecetada > 0 
				AND $sales_table.date between \"$date1\" and \"$date2\"
				AND $sales_items_table.quantity_purchased > 0 GROUP BY $sales_table.id ORDER BY $sales_table.id DESC",$this->conn);
			...
			WHERE $sales_table.id = $sales_items_table.sale_id AND $sales_table.tipo_vta = 2 AND $sales_items_table.tipo = 2 AND $sales_items_table.qtyrecetada > 0 AND $sales_table.status = 1 AND $sales_items_table.quantity_purchased > 0
			AND $sales_table.date between \"$date1\" and \"$date2\"
			...
			while($row = mysql_fetch_assoc($result)) {
				...
				$profit = 0;
				$distinct_date = $row['date'];
				$result2 = mysql_query("SELECT $sales_table.id, $sales_table.date, $sales_table.customer_id as paciente_id, ($sales_items_table.item_unit_price * $sales_items_table.quantity_purchased) AS sale_sub_total, ($sales_items_table.item_buy_price * $sales_items_table.quantity_purchased) AS sale_total_cost, $sales_table.paid_with, $sales_table.items_purchased, $sales_table.sold_by, $sales_table.comment, $sales_table.changee, $sales_table.invoicenumber, $sales_table.status, $sales_table.descuentporc, $sales_table.value_discount, $sales_table.tipo_vta 
		    FROM $sales_table,$sales_items_table 
			WHERE $sales_table.id = $sales_items_table.sale_id AND $sales_table.tipo_vta = 2 AND $sales_items_table.tipo = 2 AND $sales_items_table.qtyrecetada > 0 AND $sales_table.status = 1
			AND $sales_items_table.quantity_purchased > 0 
			AND $sales_table.date=\"$distinct_date\"
			ORDER BY $sales_table.id DESC",$this->conn);
				...
				while($row2 = mysql_fetch_assoc($result2)) {
					$amount_sold += $row2['sale_sub_total'];
					$total_amount_sold += $row2['sale_sub_total'];
					$profit += ($row2['sale_sub_total'] - $row2['sale_total_cost']);
					$total_profit += ($row2['sale_sub_total'] - $row2['sale_total_cost']);
				}
				$porcmargen = number_format(($profit / $amount_sold) * 100,2,'.','');
				$amount_sold = number_format($amount_sold,2,'.','');
				$profit = number_format($profit,2,'.','');
			...
			$total_profit = number_format($total_profit,2,'.','');
			echo "<br><table align='right' cellspacing='$this->cellspacing' cellpadding='$this->cellpadding' bgcolor='$this->table_bgcolor' width='60%' border=0>";
			echo "<tr><td>{$this->lang->totalAmountSoldDis}: <b>$this->currency_symbol$total_amount_sold</b></td></tr>
			 	<tr><td>{$this->lang->totalProfit}: <b>$this->currency_symbol$total_profit</b></td></tr>
				 </table>";
		} elseif($total_type == 'valpvpcost') {
			echo "<table align='center' cellspacing='$this->cellspacing' cellpadding='$this->cellpadding' bgcolor='$this->table_bgcolor' width='40%' style=\"border: $this->border_style $this->border_color $this->border_width px\">";
