<?php class display
{
		...
		$wherefield,$wheredata,$date1,$date2,$orderby,$subtitle)
	{
		...
		if ($tablename == 'sales_items') {
			...
			if ($wherefield != '' and $wheredata != '' and $date1 == '' and $date2 == '') {
				...
				WHERE $table.id = $sales_items_table.sale_id AND $table.tipo_vta = 2 AND $sales_items_table.tipo = 2 
	...
	function displaySaleManagerTable($tableprefix,$where1,$where2)
	{
		...
		if ($where1 != '' and $where2 != '') {
			...
			$sale_query = "SELECT * FROM $sales_table WHERE tipo_vta = 1 and id between \"$where1\" and \"$where2\" ORDER BY id DESC";
			$sale_result = mysql_query($sale_query,$this->conn);

		} else {
			$sale_query = "SELECT * FROM $sales_table WHERE tipo_vta = 1 ORDER BY id DESC";
			$sale_result = mysql_query($sale_query,$this->conn);
		...
		$where1,$where2)
	{
		...
		if ($total_type == 'customers') {
			...
			while ($row = mysql_fetch_assoc($employee_result)) {
				...
				$query2 = "SELECT * FROM $sales_table WHERE tipo_vta=1 AND sold_by=\"$temp_empl_id\" and date between \"$date1\" and \"$date2\"";
				$result2 = mysql_query($query2,$this->conn);
			...
			$query = "SELECT DISTINCT date FROM $sales_table WHERE tipo_vta=1 AND date between \"$date1\" and \"$date2\" ORDER by date ASC";
			...
			while ($row = mysql_fetch_assoc($result)) {
				...
				$result2 = mysql_query("SELECT * FROM $sales_table WHERE tipo_vta=1 AND date=\"$distinct_date\"",
					$this->conn);
