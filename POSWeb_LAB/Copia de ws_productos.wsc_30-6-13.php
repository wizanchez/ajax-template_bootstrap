...
define(URL_WEB_ws,'http://191.168.0.56/emssanar/');
			...
			##	WEB SERVICE EN TABLAS ORIGINALES
				define(URL_WEB_SERVICE_SERVIDOR,URL_WEB_ws.'v3.01/oserp/ws_asistencial_v20.wsdl.php');
			#############################################################################################################


define(FECHA_INICIO_SINCRONIZACION,date('2011-01-01'));
...
class ws_productos{
	...
	public function consultarProductos($ws_url,$value='',$tipo='',$bodega_id='',$proveedor_id='',$incluir_lib='')
	{
		...
		$arreglo= array('tipo' => $tipo,'value' => $value,'bodega'=>$bodega_id,'proveedor'=>$proveedor_id,'acceso'=>ws_productos::clave());
	...
	public function sincronizarPreciosProductos($value='',$tipo='',$bodega_id='',$proveedor_id='',$incluir_lib='')
	{
		global $cliente;
		$a_productos		=array();	
...
//		$ws_url			='http://suiteerp.com/emssanar/v3.01/oserp/ws_asistencial.wsdl.php';
		...
		$a_productos	=ws_productos::consultarProductos($ws_url,$value,$tipo,$bodega_id,$proveedor_id,$incluir_lib);
		//echo '===============================';#Omar:>
		//echo'<pre>'; print_r($a_productos);return false;
			...
			$a_vec['buy_price']			=$a_productos[0]['bodega'][0]['ultimo_costo'];
			$a_vec['unit_price']		=$a_productos[0]['bodega'][0]['precio'][0]['precio'];
			$a_vec['total_cost']		=$a_productos[0]['bodega'][0]['precio'][0]['precio'];
			...
			## actualizamos los comvenios	
				for($p=0;$p<count($a_productos[0]['bodega'][0]['convenios']);++$p){
						...
						if(count($a_info_item_convenio)==0){
										...
										AdminConsulta::insertar2($a_vec2, $tabla=TABLA_POS_ITEMCONVENIO, $ver_query='');//ver_consulta
