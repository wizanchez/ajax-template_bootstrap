...
*/
	  $onclik=	($MOTRAR_CLICK===true) ?'':"alert('No Puede Pedir este Item No esta dentro del Convenio \ Puede que despues de sincronizado tenga un nuevo convenio');";

$onclik.='fill(\''.$rc_items['item_number'].' :: '.$rc_items['shortdescription'].' $'.number_format($rc_items['unit_price'],0).' Tot  $ '.number_format($fracc,0).' Und.  Qty [ '.$rc_items['quantity'].' ]\',\''.number_format($rc_items['unit_price'],0).'\',\''.number_format($rc_items['quantity'],1,'.',',').'\');';
