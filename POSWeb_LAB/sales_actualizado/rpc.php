...
if (!$dbf) {
	...
	if (isset($_REQUEST['queryString'])) {
		...
		if (strlen($queryString) != '') {
			...
			if ($_REQUEST['tipo_vta'] == '2') {
				...
				if (DISTRI_VTA) {
					$items = $dbf->getElements('pos_items',
						'id, item_number, item_name, unit_price, pack, quantity,shortdescription','shortdescription',' LIKE "%'.$queryString.'%" and active = 1  GROUP BY item_number','3',' LIMIT 200');
				} else {
			...
			and pos_pacientes.id = "'.$_REQUEST['customer_list'].'" GROUP BY item_number','3',' LIMIT 200');
				}
			} else {
				//
				//
				...
				$items = $dbf->getElements('pos_items',
					...
					$queryString.'%" and active = 1 GROUP BY item_number','3',' LIMIT 200');
			}
			if ($items) {
