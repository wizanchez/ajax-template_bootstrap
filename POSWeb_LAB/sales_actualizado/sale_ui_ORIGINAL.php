			...
			success: function(data){
						...
						jQuery('#d_to_inv').html(data.quantity);
							...
							if(parseInt(can_old)!=parseInt(data.quantity)){
								...
								setTimeout("document.getElementById('d_to_inv').style.backgroundColor='#000'", 5000);
								}	
...
<td nowrap><input type="text" name="num_formu" id="num_formu" onChange="submitPaciente()" value="<?= $numFormula?>"/></td>                
<?php } else {?>
...
<?php }?>
                ...
                <td colspan="3" ><input type="button" value="<?= $lang->newSale?>" disabled/>&nbsp;&nbsp;&nbsp;
                <input type="button" value="<?= $lang->cancelsaleall?>" onClick=javascript:cancelsale(); />&nbsp;&nbsp;&nbsp;
                </td>
...
?>: </th>
	<td width="5%" nowrap>
   		<input type='text' name='item' id='item' onChange='searchitem(this.value,2);' size='14' value="<?php echo $strItem;?>" onClick="seleccionar_campo(this.id);">
	</td>
	<th style="vertical-align: middle; text-align: center; background-color: #A73535; " width="5%"><?=$lang->description?>: 
	<?php
				  ##############################################################
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td><?php //////////////////////////////////////////////////////////////
			if ($cfg_searchproductcombo == 1) {?>    
                <?= $item_title?>
                <select name='item_list' onChange=\"updateScanItemField()\";>
            ...
            <?php } else {$strItem = '';?>
    <input type="text" size="50" value="<?= $strItem?>" id="inputString" name="inputString" onKeyUp="lookup(this.value);" onBlur="fill();" onClick="seleccionar_campo(this.id);" autocomplete= 'off' />
	
            <?php }?> </td>
        <th style="vertical-align: middle; text-align: center; background-color: #A73535; padding-left:4px; padding-right:1px;" width="5%">Costo$</th>
        <th style="width:80px !important; text-align:right; padding-left:8px;"align="right" id="d_to_pre">&nbsp;</th>
        <th style="vertical-align: middle; text-align: center; background-color: #A73535; padding-left:4px; padding-right:1px;" width="5%">Inv</th>
        <th style="width:50px !important; text-align:center; padding-left:7px; padding-right:7px;" align="center" id="d_to_inv">&nbsp;</th>
      </tr>
    </table>
...
?>
			...
			<? if ($canc == 1)
					...
					$readon = '';
				if ($tipo_vta == '2') {?>
	<td align='center' nowrap style="text-align: right;color: white;  font-weight: bold;"><?= $tipoVenta?></td>                    
                    <?php }?>    
	<td align='center' nowrap style="text-align: right;color: white;  font-weight: bold;"><?= $temp_item_code?></td>
	<td align='left' nowrap style="text-align: right;color: white;  font-weight: bold;"><?= $resto?></td>
	<td align='center' nowrap><input type=text name='price<?= $k?>' style='text-align:center' readonly value='<?= $temp_sindsto2?>' size='8'></td>
	<td nowrap style="text-align: right;color: white;  font-weight: bold;"><input type="hidden" name="temp_discount" id="temp_discount" value="<?= $temp_discount?>" /><?= $temp_discount?><?= $lang->sigporc?></td>
	...
	<td nowrap ><input type=text name='price<?= $k?>' id='pricedcto<?= $k?>' readonly style='text-align:center' value='<?= number_format($temp_price2,0,'','');?>' size='8'></td>
	<td nowrap><input type=text name='tax<?= $k?>' style='text-align:center; width: 25px;'  readonly  value='<?= $temp_tax?>' size='5'></td>
	<td nowrap><input type=text name='valimp<?= $k?>' id='valimp<?= $k?>' style='text-align:center; width: 35px;'  readonly  value='<?= $valimp2?>' size='6'></td>
	<td align='center' nowrap ><input type=text name='price<?= $k?>' readonly style='text-align:center' value='<?= $temp_pricepub2?>' size='8'></td>
	<td nowrap>                
...
?>: 
            ...
            <tr>
	<th style="vertical-align: middle; text-align: center; background-color: #A73535; "><?= $lang->amtChange?>:</th>
	<td>
    ...
    <input type='button' id='addsale' value='<?= $lang->addSale?>' onclick='asig();'>
...
<table border='0'  cellspacing='0' cellpadding='2' align='center' class="KT_tngtable" bgcolor='<?= $table_bg?>' width="100%">
        <?php //
	if ($tipo_vta == '2') {
		$sqlTvta = " AND tipo_vta = 2";
		if ($customer_list) {
