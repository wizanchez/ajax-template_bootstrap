<?php session_start();
			define(FOLDER_RAIZ,'../');
																		...
																		##	PARA PODER FACURAR CON TODAS LAS CANTIDADES PEDIDAS EN CERO
																				define(FACTURAR_CON_TODO_INV_EN_CERO,false);
																		#####################################################
...
<? if (!$can) {
	...
	?>
            ...
            <!--<th style="vertical-align: middle; text-align: center; background-color: #A73535; " width="5%" nowrap><strong>Modo</strong></th>-->
                <td width="5%" nowrap>
                            <select name='tipo_vta' onChange="formSubmit()" id="tipo_vta"  style="width:70px;" class="forma_select_1 cajas_22">
            ...
            <?php if ($sale_id == '' and $_POST['consitem']) {
				...
				$field_names = array('date','customer_id','sold_by','invoicenumber','status','tipo_vta','formula','bodega_id');
				...
				if($maxin<$cfg_fact_desde)
					...
					$maxin=$cfg_fact_desde;
					  ...
					  if($maxin >= $tempmaxin)
					  {
					  echo "ESTA LLEGANDO AL LÍMITE DE NÚMERO DE FACTURAS DE LA RESOLUCIÓN";
					  }
				$field_data = array(date("Y-m-d"),$customer_list,$_SESSION['session_user_id'],$maxin,0,$tipo_vta,$numFormula,$cfg_locationid);
...
<table width="80%"  class="fondo_fondo">
	<tr>
		...
		<td nowrap>
        	<input type="text" size="11" name="customerDoc_list" id="customerDoc_list" value="<?=$_REQUEST['customerDoc_list']?>" onChange="searchCustomer(this.value,4)" class="cajas_22"/>
        ...
        <td colspan="3"><input type="button" value="<?=$lang->cancelsaleall?>" onClick=javascript:cancelsale();  class="display_naranja"  /></td>
    ...
    <tr>
        ...
        <th style="vertical-align: middle; text-align: center; " width="5%" nowrap >
            ...
            <?=$ObjDeco->CuadroLabels($lang->itemID.'/'.$lang->itemNumber,'sin_nombre','derecha','')?>
        </th>
		<td width="5%" >
	    ...
	    <table width="100%" border="0" cellspacing="0" cellpadding="0">
            ...
            <?php } else {$strItem = '';?>
    <input type="text"  class="cajas_22"size="45" value="<?=$strItem?>" id="inputString" name="inputString" onKeyUp="lookup(this.value);" onBlur="fill();" onClick="seleccionar_campo(this.id);" autocomplete= 'off' />
        ...
        <th style="width:80px !important; text-align:right; padding-left:8px;"align="right" id="d_to_pre" bordercolor="#003366" bgcolor="#FFFFFF">&nbsp;</th>
        <th style="vertical-align: middle; text-align: center; padding-left:4px; padding-right:1px;" width="5%">
        ...
        <script>jQuery('th[rel=tipsy_sincro_inv]').tipsy({gravity: 's' ,trigger: 'manual',html:true,fade: true});</script>
      </tr>
    </table>

	      
	</td>                  
					<td><input type='submit' value='Adicionar Producto' name='consitem' id='consitem' disabled='true'  class="display_naranja" ></td>
              <td><input type='button' value='Limpiar' name='clearfields' id='clearfields'  onclick='ClearField();' class="display_naranja"  > </td>			
            </tr>                   
...
?>    
            ...
            <? if ($tipo_vta == '2') {?>    
            	...
            	<th nowrap><?php if ($tipo_vta == '2') {?><?=$lang->disp?>&nbsp;&nbsp;<?=$lang->formu?> (P) <?php }
				?>&nbsp;&nbsp;<?=$lang->cant;?>&nbsp;&nbsp;<?=$lang->fraccion?></th>
            	<th nowrap><?=$lang->past?></th>
		...
		if ($sale_id != '' and $item != '') {
			...
			if ($tipo_vta == '2') {
				...
				if ($almacen_pac == $cfg_locationid || $almacen_pac == '999' || $almacen_pac!='') {
					...
					$rc_positems = mysql_query("select pos_items.id,pos_items.tax_percent,pos_itemconvenio.price_vta as unit_price,pos_items.pack,pos_items.buy_price,pos_items.tax_percent, pos_items.por_regulacion as regulado, pos_items.price_regulacion as precio_regulado FROM pos_items, pos_itemconvenio,pos_pacientes WHERE pos_items.item_number = '".
					...
					$rc_positems = mysql_query("select pos_items.id,pos_items.tax_percent,pos_itemconvenio.price_evento as unit_price,pos_items.pack,pos_items.buy_price,pos_items.tax_percent, pos_items.por_regulacion as regulado, pos_items.price_regulacion as precio_regulado FROM pos_items, pos_itemconvenio,pos_pacientes WHERE pos_items.item_number = '".
						$item."' and pos_items.active = 1 and pos_itemconvenio.estado = 1 and pos_items.id = pos_itemconvenio.items_id AND pos_itemconvenio.convenio_id = pos_pacientes.convenioid and pos_pacientes.id = '$customer_list'",
						$conn_2);
				}
				$num_rows = mysql_num_rows($rc_positems);
			...
			/* validamos si el producto esta regulado o no y cambiamos el precio venta por el del regulado, en caso que */
			/* el producto sea regulado de lo contrario dejamos como sigue */
			/* Modificación por Ing. Senior Felipe Camacho */
			
			if($positems['regulado'] == 1 and $temp_price2 > $positems['precio_regulado']) :
				$temp_price2 = $positems['precio_regulado'];
			endif;
...
?>
	...
	<td align='center' nowrap>
    	<input type=text name='price<?=$k?>'  class="cajas_22"style='text-align:center' readonly value='<?=$temp_sindsto2?>' size='6'>
    </td>
	<td nowrap style="text-align: center;color: #015B7E;  font-weight: bold;">
    	<input type="hidden" name="temp_discount" id="temp_discount" value="<?=$temp_discount?>" /><?=$temp_discount?><?=$lang->sigporc?>
    </td>
	<td nowrap style="text-align: center;color: #015B7E;  font-weight: bold;">
    	<input type="hidden" name="valdsto" id="valdsto" value="<?=$valdsto?>" /><?= number_format($valdsto,0,'','');?>
   </td>
	<td nowrap  style="text-align: center;">
    	<input type=text name='price<?=$k?>'  class="cajas_22"id='pricedcto<?=$k?>' readonly style='text-align:center' value='<?= number_format($temp_price2,0,'','');?>' size='6'>
    </td>
	<td nowrap style="text-align: center;">
	...
	<td align='center' nowrap  style="text-align: center;">
    	<input type=text name='price<?=$k?>' class="cajas_22" readonly style='text-align:center' value='<?=$temp_pricepub2?>' size='6'>
    </td>
	<td nowrap  style="text-align: center;">                
...
<input type=text name='dispo<?=$k?>' id='dispo[<?=$k?>]' class="cajas_22" readonly  style='text-align:center; width: 25px;' value='<?= number_format($inveQty,0,'','');?>' size='3'>

<input type=text name='orden<?=$k?>' id='orden[<?=$k?>]' class="cajas_22" style='text-align:center; width: 25px;' value='<?= number_format($row_item['qtyrecetada'],0,'','');?>' size='3' <?=$disableFormu?>  onchange='validFormulada(<?=$k?>)'> (<?= number_format($temp_quantityB,'2',',','')?>)
<? } else {
					if ($tipo_vta == '2') {
?>
...
<?php }
?>
<input type="hidden" name='dispo<?=$k?>' id='dispo[<?=$k?>]' <?=$readon?>  style='text-align:center; width: 25px;' value='<?= number_format($inveQty,0,'','');?>' size='3'>
...
(<?=$temp_pack?>)
				...
				<td nowrap align='center'><input type='hidden' name='pack<?=$k?>' id='pack[<?=$k?>]' value='<?=$temp_pack?>'/> 
...
?>        
        ...
        <thead>
            <tr> 
				...
				<th style="text-align: right;"></th>
				<th style="text-align: right;"><?= number_format($tot_valimpD)?></th>
                <th style="text-align: right;"><?= number_format($tot_temp_pricepubD)?></th>
...
?>
			...
			<tr>
                ...
                <td  class="fondo_fondo">    
                    <input type=text name='amountpay' id='amountpay' readonly style='text-align:center' class="cajas_22"   value='' size='8'>
                </td>    
            <tr>
	<th style="vertical-align: middle; text-align: center;">
	<?=$ObjDeco->CuadroLabels($lang->amtChange,'sin_nombre','derecha','')?></th>
	<td  class="fondo_fondo">
	<input type=text name='changep' id='changep' readonly style='text-align:center'   value='' size='8' class="cajas_22">
	<input type=hidden name='j' id='j'   value='$k'><input type=hidden name='cliente' id='cliente' value=''>
    <input type='button' id='addsale' value='<?= $lang->addSale?>' onclick='asig();'  class="display_naranja">
...
?>
			<table align="center" border="0" class="fondo_fondo">
			<tr>
		...
		<th style="vertical-align: middle; text-align: center;  " width="5%" nowrap><div align="center"><strong><font color='white'>
		<?=$ObjDeco->CuadroLabels_puntas_vino($lang->doc_num,'sin_nombre','centro','')?></font></strong></div></th>
        <th style="vertical-align: middle; text-align: center; " width="5%" nowrap><div align="center"><strong><font color='white'>
...
?>  
				...
				<input type=hidden name='paymethodname[$k]' id='paymethodname[$k]'   value='$name' size='8'>";?></th>
				<td><? echo "<input type=text name='amount[$k]' id='amount[$k]'   value='' size='8' onblur='cambamount(this);'  class='cajas_22'>";?></td>
				<? if ($change == 1) {?>
					<td><? echo "<input type=text name='doc_num[$k]' id='doc_num[$k]'   value='' size='8' class='cajas_22'>";?></td>
					<td><select name="<? echo "entity$k";?>" id="<? echo "entity[$k]";?>" style="width:100px;"  class="forma_select_1">	<option value="0">Elija</option>
					...
					<td><? echo "<input type=text name='auth_num[$k]' id='auth_num[$k]'  class='cajas_22'  value='' size='8'><input type='hidden' name='k' id='k' value='$k' />
...
?>
			...
			<td colspan="4" ><? echo
			"<input type='text' name='comment' id='comment' size='40'  class='cajas_22'/>";?></td>
...
<?php }?>             
	...
	<th><?=$lang->vlriva?></th>
	<th><?=$lang->pvp?></th>
	<th nowrap><?php if ($tipo_vta == '2') {?><?=$lang->disp?>&nbsp;&nbsp;<?=$lang->formu?> (P) <?php }?>&nbsp;&nbsp;<?=$lang->cant;?>&nbsp;&nbsp;<?=$lang->fraccion?></th>
	<th><?=$lang->past?></th>
	<th><?=$lang->vlrtot?></th>
	<th><?=$lang->cancel?></th>
</tr>
	...
	while ($row_item = mysql_fetch_assoc($result_posslaesitem)) {
		...
		if ($tipo_vta == '2') {
			...
			if ($num_rows > 0) {
				...
				if ($almacen_pac == $cfg_locationid || $almacen_pac == '999' || $almacen_pac!='') {
					$tipoVenta = '<span style="font-size: 13px;cursor:pointer" id="tipoDisp'.$k.'" onclick="cambioDispe('.$k.','.$row_item['tipo'].')"><strong>'.$srtEvent.'</strong></span>';
				} else {
...
?>      
				...
				<td align='center' nowrap><input type=text name='price<?=$k?>' style='text-align:center' readonly value='<?=$temp_sindsto2?>' size='6' class="cajas_22"></td>
				...
				<td nowrap style="text-align: center;">
          	<input type=text name='price<?=$k?>' readonly  class="cajas_22"style='text-align:center' id='pricedcto<?=$k?>' value='<?= number_format($temp_price2,0,'','');?>' size='6'>
            	</td>
				<td nowrap   style="text-align: center;">
                <input type=text id="tax<?=$k?>" name='tax<?=$k?>' class="cajas_22" style='text-align:center;width: 25px;'  readonly  value='<?=$temp_tax?>' size='5'></td>
				<td nowrap  style="text-align: center;">
				...
				<td nowrap style="text-align: center;">
                <input type=text name='price<?=$k?>' readonly style='text-align:center'  class="cajas_22"value='<?=$temp_pricepub2?>' size='6'></td>
				<td nowrap  style="text-align: center;">
...
?>
<input type=text name='dispo<?=$k?>' id='dispo[<?=$k?>]' readonly   class="cajas_22"style='text-align:center; width: 25px;' value='<?= number_format($inveQty,0,'','');?>' size='3'>

<input type=text name='orden<?=$k?>' id='orden[<?=$k?>]'  class="cajas_22"style='text-align:center; width: 25px;' value='<?= number_format($formu_quantity,0,'','');?>' size='3' <?=$disableFormu?> onchange='validFormulada(<?=$k?>,<?=$temp_quantity?>)'> (<?= number_format($temp_quantityB,'2',',','')?>)
<? } else {
			if ($tipo_vta == '2') {
?>
...
<input type='text' name='quantity<?=$k?>' id='qty[<?=$k?>]' onchange="validQtyInv(this.value,<?=$k?>,'<?=$temp_quantityB?>')"  <?=$readon?>  style='text-align:center; width: 25px;' value='<?= number_format($temp_quantity,0,'','');?>' size='3' onchange="updateitemsale(<?=$k?>)"> (<?=$temp_pack?>)</td>
				<td nowrap align='center'><input type='hidden' name='pack<?=$k?>' id='pack[<?=$k?>]' value='<?=$temp_pack?>'/> 
