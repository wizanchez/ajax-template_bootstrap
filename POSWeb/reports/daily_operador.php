...
if($submit==1){
	...
	$tableheaders=array("$lang->date","$lang->soldBy","$lang->saleTotalCost");
	$tablefields=array('date','sold_by','sale_total_cost');
	$todaysDate=$_REQUEST['datepicker'];
	$display->displayReportTableVentaDiariaOperador("$cfg_tableprefix",'sales',$tableheaders,$tablefields,'date',"$todaysDate",'','','invoicenumber',"Total venta por Operador");
}else{
	?>
    ...
    <form action="daily_operador.php">
