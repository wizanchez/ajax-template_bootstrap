...
if (isset($_GET['update_item']))
{
    ...
    $_SESSION['items_in_sale'][$k] = $item_id . ' ' . $new_price . ' ' . $new_tax .
        ' ' . $new_quantity . ' ' . $percentOff . ' ' . $past . ' ' . $showitem;
...
if (isset($_GET['discount']))
{
    ...
    if (is_numeric($discount))
    {
        for ($k = 0; $k < count($_SESSION['items_in_sale']); $k++)
        {
            ...
            $_SESSION['items_in_sale'][$k] = $item_id . ' ' . $new_price . ' ' . $tax . ' ' .
                $quantity . ' ' . $percentOff . ' ' . $tax . ' ' . $past . ' ' . $showitem;
        }
...
if (isset($_POST['customer']))
{
...

    {
        ...
        {
            echo "$lang->customerWithID/$lang->accountNumber " . $_POST['customer'] . ', ' .
                "$lang->isNotValid";
        }
    }
}
        ...
        if ($itemsearch_2 != '' and $item != '')
        {
            ...
            if (isset($_POST['item']))
            {
                ...
                if ($dbf->isValidItem($item))
                {
                    ...
                    $_SESSION['items_in_sale'][] = $item . ' ' . $itemPrice . ' ' . $itemTax . ' ' .
                        '1' . ' ' . $discount . ' ' . $past . ' ' . $showitem;
                    $_SESSION['sales_paywith'][] = $paymethodid . ' ' . $amount . ' ' . $doc_num .
                        ' ' . $entity . ' ' . $auth_num;
...
?>>Dispensaci&oacute;n</option>
            ...
            if ($sale_id == '' and $_POST['consitem'])
            {
                ...
                $maxin = $maxin['maxin'] + 1;
                $field_data = array(date("Y-m-d"), $customer_list, $_SESSION['session_user_id'],
                    $maxin, 0, $tipo_vta);
            ...
            $rc_saleop = mysql_query('select id,customer_id  from pos_sales where sold_by = '.$userId.' and status = 0 ' .
                $sqlTvta, $conn_2);
...
?>
            <input name="sale_id" type="hidden" id="sale_id" value="<?=
            $sale_id
?>" />
...
<td nowrap>
<input type="text" size="12" name="customerDoc_list" id="customerDoc_list" value="<?=
            $_REQUEST['customerDoc_list']
...
?>
<input type="text" name="nameCustomer" id="nameCustomer" autocomplete="off" size="80" value="<?=$customerstr?>"/>
<?php
            if ($tipo_vta == '1')
            {
?>
...
?>    
                    ...
                    <input type='submit' value='Go'><a href='delete.php?action=item_search'><font size='-1' color='white'>[<?=
                $lang->clearSearch
?>]</font></a>
            ...
            {
?> 
				<input type="text" size="15" name="itemsearch_2" id="item_search2" value="<?=
                $itemsearch_2
?>" onChange="searchitem(this.value,2);"><?
                if ($itemsearch_2 != '')
                {
?>
					<script src="../ajax/ajaxjs2.js"></script>
...
?>
                ...
                <th style="vertical-align: middle; text-align: center; background-color: #A73535; " width="5%" nowrap><?php
            echo $lang->itemID . ' / ' . $lang->itemNumber
?>: </th>
                <td width="5%" nowrap>
                    <input type='text' name='item' id='item' onChange='searchitem(this.value,2);' size='14' value="<?=
            $strItem
?>">
                 </td>
                 <th style="vertical-align: middle; text-align: center; background-color: #A73535; " width="5%"><?=
            $lang->description
?>: </th>
            <?php
            //////////////////////////////////////////////////////////////
...
?>
                        ...
                        <td width="5%" nowrap><input type="text" size="50" value="<?=
                $strItem
...
<table border='0'  cellspacing='0' cellpadding='2' align='center' class="KT_tngtable" bgcolor='<?=
        $table_bg
?>' width="100%">
        <?php
        //
        //echo 'select id  from pos_sales where status = 0 and sold_by = '.$userId.' and ' . $sqlTvta;
        $rc_saleopen = mysql_query('select id  from pos_sales where status = 0 and sold_by = '.$userId.' ' . $sqlTvta,
            $conn_2);
...
?>"/>
        ...
        if ($sale_id != '' and $item != '')
        {
            ...
            if ($tipo_vta == '2')
            {
                $rc_positemsB = mysql_query("select pos_items.id,pos_items.tax_percent,pos_itemconvenio.price_vta as unit_price,pos_items.pack,pos_items.buy_price,pos_items.tax_percent FROM pos_items, pos_itemconvenio,pos_pacientes WHERE pos_items.item_number = '" .
                    ...
                    $conn_2);
                $num_rows = mysql_num_rows($rc_positemsB);
                if ($num_rows > 0)
                {
            ...
            $field_names = array('sale_id', 'item_id', 'quantity_purchased',
                'item_unit_price', 'item_buy_price', 'item_tax_percent', 'item_total_tax',
                'item_total_cost', 'unit_sale', 'sale_frac', 'tipo', 'porcen_dcto', 'value_dcto',
                'qtyrecetada');
            $field_data = array($sale_id, $positems['id'], 1, $temp_price2, $positems['buy_price'],
                $positems['tax_percent'], $valimp, $temp_price2, 1, $positems['pack'], $itipo, $temp_discount,
                $valdsto, 0);
            ...
            while ($row_item = mysql_fetch_assoc($result_posslaesitem))
            {
                ...
                if ($row_item['id'] == $saitemid)
                {
                    ...
                    $queryupd = 'update pos_sales_items set quantity_purchased = ' . $qty .
                        ...
                        ', item_total_cost = ' . $pvptot . ', item_buy_price = ' . $prcomp .
...
?>" />
			<?
                ...
                {
?>
                <td align='center' nowrap style="text-align: right;color: white;  font-weight: bold;"><?=
                    $tipoVenta
?></td>                    
                    <?php
                }
?>    
                ...
                $temp_item_code
?></td>
				<td align='left' nowrap style="text-align: right;color: white;  font-weight: bold;"><?=
                $resto
?></td>
				<td align='center' nowrap><input type=text name='price<?=
                $k
?>' style='text-align:center' readonly value='<?=
                $temp_sindsto2
?>' size='8'></td>
...
?></td>
				<td nowrap ><input type=text name='price<?=
                $k
?>' id='pricedcto<?=
                $k
?>' readonly style='text-align:center' value='<?=
                number_format($temp_price2, 0, '', '');
?>' size='8'></td>
				<td nowrap><input type=text name='tax<?=
                $k
?>' style='text-align:center; width: 25px;'  readonly  value='<?=
                $temp_tax
?>' size='5'></td>
				<td nowrap><input type=text name='valimp<?=
                $k
?>' id='valimp<?=
                $k
?>' style='text-align:center; width: 35px;'  readonly  value='<?=
                $valimp2
?>' size='6'></td>
				<td align='center' nowrap ><input type=text name='price<?=
                $k
...
?>]' <?=
                    $readon
?>  style='text-align:center; width: 25px;' value='<?=
                    number_format($inveQty, 0, '', '');
?>' size='3'>
...
?>' id='orden[<?=
                    $k
?>]' style='text-align:center; width: 25px;' value='<?=
                    number_format($row_item['qtyrecetada'], 0, '', '');
?>' size='3'  onchange='validFormulada(<?=
                    $k
?>)'>
<?
                } else
...
?>]' <?=
                $readon
?> onchange="validQtyInv(this.value,<?=$k?>)" style='text-align:center; width: 25px;' value='<?=
                number_format($temp_quantity, 0, '', '');
?>' size='3' onchange="updateitemsale(<?=
                $k
?>)" >
(<?=
                $temp_pack
?>)
				...
				<td nowrap align='center'><input type='hidden' name='pack<?=
                $k
?>' id='pack[<?=
                $k
?>]' value='<?=
...
?>' id='past[<?=
                $k
?>]' onchange="updateitemsale(<?=
                $k
?>)">
                        <option value="1"
            <?php
                if (!(strcmp("1", $temp_packing)))
                {
...
?>
				...
				<td align='center' nowrap><font color='white'><b><?=
                $cfg_currency_symbol . number_format($vlrtot, 0)
?></b></font>
				<input type='hidden' name='temp_pricepub2<?=
                $k
?>' id='pvpfin[<?=
                $k
?>]' value='<?=
                $temp_price2
?>'/>
				<input type='hidden' name='preciocompra<?=
                $k
?>' id='preciocompra[<?=
                $k
?>]' value='<?=
...
?>]' value='<?=
                    $temp_item_id
?>'>
				<input type='hidden' name='idpossalesitems<?=
                    $k
?>' id='idpossalesitems[<?=
                    $k
?>]' value='<?=
...
?>'>
                ...
                <img src="../images/cancel.png" border="0" title="<?=
                    $lang->cancel
?>"   onClick=javascript:cancelitemsale(<?=
                    $k
?>);  style="cursor:pointer"/>
                <input type="hidden" value="1" name="ItemOn" id="ItemOn<?=
                    $k
?>" />
...
?>
                ...
                <input type="hidden" value="0" name="ItemOn" id="ItemOn<?=
                    $k
?>" />
...
?>
                <input type='hidden' name='showitem<?=
                $k
?>' id='showitem<?=
                $k
?>' value='<?=
            ...
            if ($number)
            {
                while ($row4 = mysql_fetch_assoc($rc_tax))
                {
                    for ($k = 0; $k < $num_items; $k++)
                    {
                        ...
                        {
                            $taxa += $aa[$k];
                            echo "<input type=hidden name='taxid" . $row4["tax_rates_id"] .
                                "' id=''   value='" . $row4["tax_rates_id"] . "'>";
...
?></th>
            ...
            number_format($tot_temp_sindstoD, 0)
?></th>
				<th style="text-align: right;"></th>
				<th style="text-align: right;"><?=
            number_format($tot_valdstoD, 0)
?></th>
				<th style="text-align: right;"><?=
            number_format($tot_temp_priceD, 0)
?></th>
				<th style="text-align: right;"></th>
				<th style="text-align: right;"><?=
            number_format($tot_valimpD)
?></th>
                <th style="text-align: right;"><?=
            number_format($tot_temp_pricepubD)
?></th>
				<th style="text-align: right;"><?=
            $qtyProD
?></th>
...
?></th>
				<th style="text-align: right;"><b><?=
        number_format($tot_temp_sindsto, 0)
?></th>
				<th style="text-align: right;"></th>
				<th style="text-align: right;"><?=
        number_format($tot_valdsto, 0)
?></th>
				<th style="text-align: right;"><?=
        number_format($tot_temp_price, 0)
?></th>
				<th style="text-align: right;"></th>
				<th style="text-align: right;"><?=
        number_format($tot_valimp)
?></th>
                <th style="text-align: right;"><?=
        number_format($tot_temp_pricepub)
?></th>
				<th style="text-align: right;"><?=
        $qtyPro
?></th>
...
?>" name="totLin" id="totLin" />
                <input name="totvta" type="hidden" id="totvta" value="<?=
        $tot_temp_pricepub
?>"/>
            		<input name="tots" type="hidden" id="tots" value="<?=
        $tot_rowTotal
?>"/>
                    <input name="customerid" type="hidden" id="customerid" value="<?=
        $customer_list
?>"/>
                    <input name="devolucionid" type="hidden" id="devolucionid" value="<?=
        $devolucion
?>"/>
...
?>
            ...
            echo "<input type='button' name='updateQuantity$k' disabled='true' id='updqty' value='$lang->update'  onclick='retval();'>
...
?>
			...
			<tr>
                <th align='left' style="vertical-align: middle; text-align: center; background-color: #A73535; "><?=
            $lang->saleSubTotal
?>:
                </th>
                <td align="center">
                    <span style="font-size: 24; color: blue; text-align: center"><strong><?=
            $cfg_currency_symbol
...
?>
                  ...
                  <input name="subtotalvta" type="hidden" id="stvta" size="6" style="text-align:center" readonly maxlength="8" value="<?=
            $finalSubTotal
?>"   /><input type="hidden" name="sub_tot" id="sub_tot" value="<?=
            $finalSubTotal
?>" />
                </td>
            </tr>
...
?>: 
            ...
            $cfg_currency_symbol
?>
<input name="taxtot" type="text" id="taxtot" size="4" style="text-align:center" readonly maxlength="8" value="<?=
            $finalTax
?>"   /><input type="hidden" name="imp_tot" id="imp_tot" value="<?=
            $finalTax
?>" />
                </td>
            </tr>
...
?>: 
            ...
            $cfg_currency_symbol
?>
                <input name="descuentshow" type="text" id="descuentshow" size="6" style="text-align:center" readonly maxlength="8" value="<?=
            number_format($descuent)
?>"   />
...
?></strong></span><strong><span style="font-size: 24; color: blue; text-align: center" id="str_saleTotalCost">
            ...
            $tot_rowTotal
?></span></strong>
                <input name="saleTotalCost" type="hidden" id="saleTotalCost" size="6" style="text-align:center" readonly maxlength="6" value="<?=
            $tot_rowTotal
?>"/>
                </td>
            </tr>
...
?>:
                ...
                <input type=hidden name='j' id='j'   value='$k'><input type=hidden name='cliente' id='cliente'   value=''><input type='button' id='addsale' value='<?=
            $lang->addSale
...
?>
			...
			<tr>
			<th style="vertical-align: middle; text-align: center; background-color: #A73535; " width="5%" nowrap><div align="center"><strong><font color='white'><?
            echo "$lang->paidWith";
?></font></strong></div></th>
			<th style="vertical-align: middle; text-align: center; background-color: #A73535; " width="5%" nowrap><div align="center"><strong><font color='white'><?
            echo "$lang->amount";
...
?></th>
				<td><?
                echo "<input type=text name='amount[$k]' id='amount[$k]'   value='' size='8' onblur='cambamount(this);'>";
?></td>
				<?
                if ($change == 1)
                {
?>
					<td><?
                    echo "<input type=text name='doc_num[$k]' id='doc_num[$k]'   value='' size='8'>";
?></td>
					<td><select name="<?
                    echo "entity$k";
?>" id="<?
...
?>
					...
					<td><?
                    echo "<input type=text name='auth_num[$k]' id='auth_num[$k]'   value='' size='8'><input type='hidden' name='k' id='k' value='$k' />
...
?>
			<tr>
			<th style="vertical-align: middle; text-align: center; background-color: #CDC442; " width="5%" nowrap><?
            echo "$lang->saleComment";
?></th>
			<td colspan="4" ><?
            echo "<input type='text' name='comment' id='comment' size='40' />";
?></td>
...
?>
    ...
    $f1->createInputField("<b>$lang->lastName :</b> ", 'text', 'last_name', "$last_name_value",
        '24', '150');
    $f1->createInputField("<b>$lang->accountNumber :</b> ", 'text', 'account_number', "$account_number_value",
        '24', '150');
    $f1->createInputField("<b>$lang->phoneNumber :</b> ", 'text', 'phone_number', "$phone_number_value",
        '24', '150');
    $f1->createInputField("$lang->email :", 'text', 'email', "$email_value", '24',
        '150');
    ...
    if ($act != 1)
    {
        ...
        $f1->createInputField("<b>$lang->lastName :</b> ", 'text', 'last_name_medico',
            "", '24', '150');
        $f1->createInputField("$lang->accountNumber : ", 'text', 'cedula_medico', "",
            '24', '150');
        $f1->createInputField("<b>$lang->phoneNumber :</b> ", 'text',
            'phone_number_medico', "", '24', '150');
        $f1->createInputField("$lang->direccioncons :", 'text', 'dicons', "", '40',
            '150');
...
?></b></td>
		...
		<?
        $rc_itemssales = mysql_query('select * from pos_sales_items where sale_id= ' . $saleID .
            '', $conn_2);
        $b = 0;
...
?>]">
				...
				<td align="center"><input type="text" name="cada[<?=
                $b
?>]" size="2" id="cada[<?=
                $b
?>]" <?=
...
?>]">
				...
				<td align="center"><input type="text" name="durante[<?=
                $b
?>]" size="2" id="durante[<?=
                $b
?>]" <?=
...
?>
            <script>
                ...
                window.opener.document.getElementById('customerDoc_list').value = <?=
            $account_number
?>;
                window.opener.searchCustomer(<?=
            $account_number
?>,4);                
                window.close();
            </script>
        ...
        if (mysql_query("INSERT INTO pos_customers (first_name, last_name, account_number, phone_number, email, street_address, comments) VALUES ('" .
            ...
            "','" . $email . "','" . $street_address . "','" . $comments . "')", $conn_2)
            === false)
        {
            $gh = 1;
        ...
        if ($act != 1)
        {
            ...
            if (mysql_query("INSERT INTO pos_medical (first_name, last_name, cedula,phone, dir_consult) VALUES ('" .
                $first_name_medico . "','" . $last_name_medico . "','" . $cedula_medico . "','" .
                $phone_number_medico . "','" . $dicons . "')", $conn_2) === false)
            {
                $gh = 1;
...
?>' width="100%">
    ...
    $rc_saleopen = mysql_query('select id  from pos_sales where sold_by = '.$userId.' and status = 0  ' . $sqlTvta,
        $conn_2);
    $saleopen = mysql_fetch_assoc($rc_saleopen);
    if ($saleopen['id'] != '')
        $sale_id = $saleopen['id'];
...
?></th>
    ...
    if ($sale_id != '' and $item != '')
    {
        ...
        $field_names = array('sale_id', 'item_id', 'quantity_purchased',
            'item_unit_price', 'item_buy_price', 'item_tax_percent', 'item_total_tax',
            'item_total_cost', 'unit_sale', 'sale_frac');
    ...
    while ($row_item = mysql_fetch_assoc($result_posslaesitem))
    {
        ...
        if ($row_item['id'] == $saitemid)
        {
            ...
            $queryupd = 'update pos_sales_items set quantity_purchased = ' . $qty .
                ', unit_sale = ' . $tipopast . ', item_unit_price = ' . $precunit .
                ', item_total_cost = ' . $pvptot . ', item_buy_price = ' . $prcomp .
                ',qtyrecetada = ' . $_REQUEST['qtyreceta' . $cont] . ' where id = ' . $saitemid .
...
?>' style='text-align:center' readonly value='<?=
        $temp_sindsto2
?>' size='8'></td>
				<td nowrap style="text-align: right;color: white;  font-weight: bold;">0 <?=
        $lang->sigporc
?></td>
				<td nowrap style="text-align: right;color: white;  font-weight: bold;"><?=
        number_format($valdsto, 0, '', '');
?></td>
				<td nowrap ><input type=text name='price<?=
        $k
?>' readonly style='text-align:center' id='pricedcto<?=
        $k
?>' value='<?=
        number_format($temp_price2, 0, '', '');
?>' size='8'></td>
				<td nowrap><input type=text id="tax<?=
        $k
?>" name='tax<?=
        $k
?>' style='text-align:center;width: 25px;'  readonly  value='<?=
        $temp_tax
?>' size='5'></td>
				<td nowrap><input type=text id="valimp<?=
        $k
?>" name='valimp<?=
        $k
?>' style='text-align:center;width: 35px;'  readonly  value='<?=
...
?>' id='orden[<?=
            $k
?>]' style='text-align:center; width: 25px;' value='<?=
            number_format($formu_quantity, 0, '', '');
?>' size='3'  onchange='validFormulada(<?=
            $k
?>)'>
<?
...
if ($actionID == 'modifiItem')
{
    ...
    echo $queryupd = 'update pos_sales_items set item_unit_price = ' . $temp_price .
        ', item_total_tax = ' . $taximpo . ',quantity_purchased = ' . $qtyPedido .
        ', qtyrecetada = ' . $_REQUEST['qtyreceta'] . ' ' . $strTipo . '  where id = ' .
        $saitemid . ' ';
