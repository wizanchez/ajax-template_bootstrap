...
$tipoReci = mysql_query("SELECT tipo_vta FROM pos_sales WHERE id = $sale_id",$dbf->
	conn);
...
if ($tipo_f['tipo_vta'] == '1') {
	...
	$tipoFp = 2;
	$tipoVtaItems = mysql_query("SELECT id FROM pos_sales_items WHERE sale_id = $sale_id and (tipo = 1 AND qtyrecetada = 0) OR (tipo in (2,3) AND qtyrecetada = 0) limit 1",
		$dbf->conn);
	if (mysql_num_rows($tipoVtaItems) > 0) {
		$result = mysql_query("select id from $customers_table where account_number = ".
			$dbf->idToField('pos_pacientes','account_number',$cliente)."",$dbf->conn);

		if (mysql_numrows($result) == 0) {
			$field_paywith = array('first_name','last_name','account_number','phone_number',
				'email','street_address');
			$field_datapay = array($dbf->idToField('pos_pacientes','first_name',$cliente),$dbf->
				idToField('pos_pacientes','last_name',$cliente),$dbf->idToField('pos_pacientes',
				'account_number',$cliente),$dbf->idToField('pos_pacientes','phone_number',$cliente),
				$dbf->idToField('pos_pacientes','email',$cliente),$dbf->idToField('pos_pacientes',
				'street_address',$cliente));
...
if ($tipoFor == '1') {
	if ($tipoFp == '2') {
		$reId = mysql_query("SELECT id FROM $customers_table WHERE account_number = ".$dbf->
			idToField('pos_pacientes','account_number',$cliente)."",$dbf->conn);
	...
	$customer_name = $dbf->idToField($customers_table,'first_name',$clienteB).' '.$dbf->
		idToField($customers_table,'last_name',$clienteB);
	...
	for ($m = 0; $m < count($amount); $m++) {
		...
		if ($amount[$m] > 0) {
			...
			$result = mysql_query("select id from $paywith_table where sale_id = ".$sale_id.
				" AND paymethod_id = ".$paymethodid[$m]." AND entity = ".$banco."",$dbf->conn);

			if (mysql_numrows($result) == 0) {
				$field_paywith = array('sale_id','paymethod_id','amount','doc_number','entity',
					'authorized_num','cancel','entrydate','entryuserid','paymethoddetail_id');
				$field_datapay = array($sale_id,$paymethodid[$m],$amount[$m],$doc_num[$m],$banco,
					$auth_num[$m],0,date("Y-m-d"),$_SESSION['session_user_id'],$entity[$m]);
...
<table  width="10px" border="0">
    <tr>
        ...
        <span><strong><?= strtoupper($cfg_company)
?></span><br />
        <span><?= strtoupper($lang->nit)
?>:</strong> <?= strtoupper($cfg_company_nit)
?></span>
        </td>
    </tr>
    <tr>
        <td nowrap style="font-size: 8px;line-height: 10px; font-family: Arial, serif; " align="left" colspan="6">
        <span ><strong><?= strtoupper($lang->city)
?>: </strong><?= $cfg_ciudad
?> <?= nl2br($cfg_address)
?></span><br />
...
?>
    <span><strong><?= strtoupper($lang->phoneNumber)
?>: </strong><?= $cfg_phone
?> <strong><?= strtoupper($lang->fax)
?>:</strong> <?= $cfg_fax
?></span><br />
<?php }
?>
<?php if ($cfg_email != '') {
?>   
    <span><strong><?= strtoupper($lang->email)
?>: </strong><?= $cfg_email
?> <strong><br /> 
<?php }
?>
<?php if ($cfg_website != '') {
?>   
    <span><strong><?= strtoupper($lang->website)
?>: </strong><a href="<?= $cfg_website
?>"><?= $cfg_website
?></a><br /> 
<?php }
?>       
        <span>
        <strong><?= strtoupper($lang->ticketnum)
?>: </strong><?= $cfg_pref_fac
?> - <?= str_pad($num_factura,6,"0",STR_PAD_LEFT);
?>
        </span><br />
        <span><strong><?= strtoupper($lang->cashier)
?>: </strong><?= strtoupper($NameUser)
?></span><br />
        <span ><strong><?= strtoupper($lang->orderBy)
?>: </strong><?= $customer_codigo
?> <?= $customer_name
?></span><br />
        <span ><strong><?= strtoupper($lang->date)
?>: </strong><?= date("Y-m-d")
?></span><span style="float: right;"><strong><?= strtoupper($lang->hoursec)
?>: </strong><?= date('h:m:s')
?></span>
    ...
    <tr>
        ...
        <strong><?= strtoupper($lang->itemID)
?></strong>
        </td>
        <td nowrap style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: center;">
        <strong><?= strtoupper($lang->sentencelong)
?></strong>
        </td>
        <td nowrap style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: center;">
        <strong><?= strtoupper($lang->priceuni)
?></strong>
        </td>
        <td nowrap style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: center;">
        <strong><?= strtoupper($lang->qty_tit)
?></strong>
        <td nowrap style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: center;">
        <strong><?= strtoupper($lang->dsto)
?></strong>        
        </td>         
        <td nowrap style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: center;">
        <strong><?= strtoupper($lang->tot)
?></strong>        
        </td>        
    </tr>
<?php //ME TRAIGO TODOS LOS PRODUCTOS DE LA FACTURA CABECERA QUE SEAN DE TIPO VTA

	$result = mysql_query("SELECT * FROM $sales_items_table  where (sale_id = $sale_id AND  tipo = 1 AND qtyrecetada = 0 AND quantity_purchased > 0) OR (sale_id = $sale_id AND  tipo = 2 AND qtyrecetada = 0 AND quantity_purchased > 0) ",
		$dbf->conn);
...
?>
    <tr>
        <td nowrap nowrap style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: left;"><?= $itemnumber
?></td>
        <td nowrap nowrap style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: left;"><?= $itemdesc
?></td>
        <td nowrap nowrap style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: right;"><?= number_format($itemprice +
		$itemdcto,'0','','.')
?></td>
        <td nowrap nowrap style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: right;"><?= number_format($itemqty,
		'0','','.')
?> <?= $itemCF
?></td>
        <td nowrap nowrap style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: right;"><?= number_format($itempdcto,
		'0','','.')
?>%</td>        
        <td nowrap nowrap style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: right;"><?= number_format($itemtotal,
		'0','','.')
?></td>
    </tr>
...
?>
    ...
    <tr>
        <td nowrap style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: center;">
        <strong><?= strtoupper($lang->tqty_tit)
?></strong>
        </td>
        <td nowrap style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: center;">
        <strong><?= strtoupper($lang->extendedPrice)
?></strong>
        </td>
        <td nowrap style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: center;" nowrap>
        <strong><?= strtoupper($lang->dstoI)
?></strong>
        </td>
        <td nowrap style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: center;" nowrap>
        <strong><?= strtoupper($lang->dstoF)
?></strong>        
...
?>   
        ...
        <?= number_format(($TotalPrice - $TotalDcto - $descuent + $Totalimpto),
	'0','','.')
?>        
        </td>  
    </tr>
    ...
    <tr>
        <td nowrap colspan="6" style="font-size: 9px;line-height: 10px; font-family: Arial, serif; " align="center" nowrap><strong><?= str_pad($lang->
	dettax,50,"-",STR_PAD_BOTH);
?></strong>
        </td>
    <tr>
        <td nowrap colspan="6">        
        <table width="100%">
        <tr>
            <td nowrap style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: center;"><strong><?= strtoupper($lang->
	tar)
?></strong></td>
            <td nowrap style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: center;"><strong><?= strtoupper($lang->
	sale)
...
?></strong></td>
	...
	$result = mysql_query("SELECT item_tax_percent FROM $sales_items_table  where (sale_id = $sale_id AND  tipo = 1 AND qtyrecetada = 0) OR (sale_id = $sale_id AND  tipo = 2 AND qtyrecetada = 0) AND quantity_purchased > 0  GROUP BY item_tax_percent ",
		$dbf->conn);
	while ($row = mysql_fetch_assoc($result)) {

		$resultB = mysql_query("SELECT sum((item_unit_price * (item_tax_percent/100)) * (quantity_purchased)) as iva, sum(item_unit_price * quantity_purchased) as base FROM $sales_items_table  where (sale_id = $sale_id AND item_tax_percent = ".
			trim($row['item_tax_percent']).
			" and tipo = 1 AND qtyrecetada = 0) OR (sale_id = $sale_id AND item_tax_percent = ".
			trim($row['item_tax_percent'])." and tipo = 2 AND qtyrecetada = 0)",$dbf->conn);
...
?>
    ...
    <tr>
        <td nowrap colspan="6" style="font-size: 9px;line-height: 10px; font-family: Arial, serif; " align="center" nowrap><strong><?= str_pad(strtoupper
	($lang->paidWithdet),50,"-",STR_PAD_BOTH);
?></strong>
        </td>
    <tr>
    <tr>
        <td nowrap colspan="2" style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: center;"><strong><?= strtoupper($lang->
	type)
?></strong></td>
        <td nowrap colspan="2" style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: center;"><strong><?= strtoupper($lang->
	entity)
...
<?php $result = mysql_query("SELECT * FROM $paywith_table WHERE sale_id = '$sale_id'",
	$dbf->conn);
	while ($row = mysql_fetch_assoc($result)) {
		$rtype = mysql_query("SELECT name FROM pos_pay_method WHERE id = '".$row['paymethod_id'].
			"'",$dbf->conn);
		$rowt = mysql_fetch_assoc($rtype);

		$rentity = mysql_query("SELECT pos_pay_methoddetail.name FROM pos_pay_method, pos_pay_methoddetail WHERE pos_pay_method.id = pos_pay_methoddetail.paymethodid AND pos_pay_methoddetail.id = '".
			$row['entity']."'",$dbf->conn);
		$rowe = mysql_fetch_assoc($rentity);
?>    
    <tr>
        <td nowrap colspan="2" style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: left;"><?= $rowt['name']
?></td>
        <td nowrap colspan="2" style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: left;"><?= $rowe['name']
?></td>
        <td nowrap colspan="2" style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: right;"><?= $row['amount']
?></td>        
    </tr>   
...
?>
    <tr>
        <td nowrap colspan="5" style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: center;"><strong><?= strtoupper($lang->
	amtChange)
?></strong>
        </td>
        <td nowrap style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: right;">
...
<tr>
        ...
        <?php if ($cfg_res_fact != '') {
?>
    <span><strong><?= strtoupper($lang->res_fact)
?> </strong></span><br /><span><strong><?= strtoupper($cfg_res_fact)
?></strong></span><br />
    <span><strong><?= strtoupper($lang->desdeno)
?>: </strong><strong><?= $cfg_pref_fac
?> - <?= str_pad($cfg_fact_desde,6,"0",STR_PAD_LEFT)
?> </strong><strong><?= strtoupper($lang->hastano)
?>:</strong> <strong><?= $cfg_pref_fac
?> - <?= str_pad($cfg_fac_hasta,6,"0",STR_PAD_LEFT)
?></strong></span><br />
<?php }
?>        
        <span>
...
if ($tipoFp == '2') {
	$result = mysql_query("SELECT id FROM $sales_items_table  where sale_id = $sale_id AND tipo = 2 AND qtyrecetada > 0 AND quantity_purchased > 0 ",
		$dbf->conn);

	if (mysql_num_rows($result) > 0) {

		$customer_name = $dbf->idToField('pos_pacientes','first_name',$cliente).' '.$dbf->
			idToField('pos_pacientes','last_name',$cliente);
...
?>: </strong><?= strtoupper($NameUser)
?></span><br />
        <span ><strong><?= strtoupper($lang->Paciente)
?>: </strong><?= $customer_codigo
?> <?= $customer_name
?></span><br />
        <span ><strong><?= strtoupper($lang->convenio)
?>: </strong><?= $dbf->idToField('pos_convenios','convenom',$convenioid)
?></span><span style="float: right;"><strong><?= strtoupper($lang->convenum)
?>: </strong><?= $dbf->idToField('pos_convenios','convenum',$convenioid)
?></span><br />
        <span ><strong><?= strtoupper($lang->date)
?>: </strong><?= date("Y-m-d")
?></span><span style="float: right;"><strong><?= strtoupper($lang->hoursec)
...
?></strong>
        ...
        <td nowrap style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: center;">
        <strong><?= strtoupper($lang->qty_for)
?></strong>
                <td nowrap style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: center;">
        <strong><?= strtoupper($lang->qty_des)
?></strong>
        <td nowrap style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: center;">
        <strong><?= strtoupper($lang->dsto)
?></strong>        
...
<?php //ME TRAIGO TODOS LOS PRODUCTOS DE LA FACTURA CABECERA QUE SEAN DE TIPO VTA
		$result = mysql_query("SELECT * FROM $sales_items_table  where sale_id = $sale_id AND tipo = 2 AND qtyrecetada > 0 AND quantity_purchased > 0 ",
			$dbf->conn);
		while ($row = mysql_fetch_assoc($result)) {
			//ACTUALIZACION DE INVENTARIO
			$sql = "UPDATE pos_items SET quantity = (quantity - ".$row['quantity_purchased'].
			...
			$itemdcto = $row['value_dcto'];
			$itempdcto = $row['porcen_dcto'];
			$itemtotal = ($itemprice + number_format($row['item_total_tax'],'0','','.')) * $itemqty;
			if ($row["unit_sale"] == 1)
				$itemCF = 'C(s)';
			else
				$itemCF = 'F(s)';
...
?></td>
        <td nowrap nowrap style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: right;"><?= number_format($itemprice +
			$itemdcto,'0','','.')
?></td>
        <td nowrap nowrap style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: right;"><?= number_format($itemqtyreceta,
			'0','','.')
?> <?= $itemCF
?></td>
        <td nowrap nowrap style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: right;"><?= number_format($itemqty,
...
<?php $TotalCant += $itemqty;
			$TotalPrice += (($itemprice + $itemdcto) * $itemqty);
			$TotalDcto += ($itemdcto * $itemqty);
			$Totalimpto += ($row['item_total_tax'] * $itemqty);
		}
?>
    <tr>
...
?></strong></td>
        ...
        <? //ME TRAIGO LOS IMPUESTOS
		$result = mysql_query("SELECT item_tax_percent FROM $sales_items_table  where sale_id = $sale_id GROUP BY item_tax_percent ",
			$dbf->conn);
		while ($row = mysql_fetch_assoc($result)) {
			$resultB = mysql_query("SELECT sum(item_total_tax * quantity_purchased) as iva, sum(item_unit_price * quantity_purchased) as base FROM $sales_items_table  where sale_id = $sale_id AND item_tax_percent = ".
				trim($row['item_tax_percent'])."",$dbf->conn);
			while ($rowb = mysql_fetch_assoc($resultB)) {
?>
        <tr>
            <td nowrap style="font-size: 8px;line-height: 10px; font-family: Arial, serif; text-align: left;">
...
<?php }
	...
	$result = mysql_query("SELECT id FROM $sales_items_table  where sale_id = $sale_id AND tipo = 3 AND qtyrecetada > 0 AND quantity_purchased > 0 ",
		$dbf->conn);
...
<?php //ME TRAIGO TODOS LOS PRODUCTOS DE LA FACTURA CABECERA QUE SEAN DE TIPO VTA
		$result = mysql_query("SELECT * FROM $sales_items_table  where sale_id = $sale_id AND tipo = 3 AND qtyrecetada > 0 AND quantity_purchased > 0 ",
			$dbf->conn);
		while ($row = mysql_fetch_assoc($result)) {
			//ACTUALIZACION DE INVENTARIO
			$sql = "UPDATE pos_items SET quantity = (quantity - ".$row['quantity_purchased'].
...
'", sale_total_cost = "'.$saleTotalCost.'", changee = "'.$changep.
	'", status = 1, customer_id = '.$cliente.', descuentporc = '.$descuentporc.
	', value_discount = '.$descuent.' where id = '.$sale_id.'';
...
window.open("sale_ui.php?actionID=addcustomer&titulo=2&saleID=<?= $sale_id
