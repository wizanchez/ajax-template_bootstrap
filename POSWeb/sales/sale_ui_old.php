...
<td nowrap>
<input type="text" size="12" name="customerDoc_list" id="customerDoc_list" value="<?= 			$_REQUEST['customerDoc_list']
...
?>' size='8'></td>
				<td nowrap style="text-align: right;color: white;  font-weight: bold;"><?= 				$temp_discount
?> <?= 				$lang->sigporc
?></td>
...
?>]' style='text-align:center; width: 25px;' value='<?= 					number_format($row_item['qtyrecetada'],
						0,'','');
?>' size='3'  onchange='validFormulada(<?= 					$k
?>)'>
<? 				} else {
					if ($tipo_vta == '2') {
?>
...
?>"/>
	...
	while ($row_item = mysql_fetch_assoc($result_posslaesitem)) {
		...
		if ($row_item['id'] == $saitemid) {
			...
			$queryupd = 'update pos_sales_items set quantity_purchased = '.$qty.
				', unit_sale = '.$tipopast.', item_unit_price = '.$precunit.
				', item_total_tax = '.$valimpN*$qty.',item_total_cost = '.$pvptot.', item_buy_price = '.$prcomp.
...
if ($actionID == 'modifiItem') {
	...
	echo $queryupd = 'update pos_sales_items set item_unit_price = '.$temp_price.
		', item_total_tax = '.$taximpo*$qtyPedido.',item_total_tax = '.($temp_price+$taximpo)*$qtyPedido.',quantity_purchased = '.$qtyPedido.
		', qtyrecetada = '.$_REQUEST['qtyreceta'].' '.$strTipo.'  where id = '.$saitemid.
		' ';
