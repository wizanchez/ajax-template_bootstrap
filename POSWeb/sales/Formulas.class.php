...

{
	...
	*/
	...
	public function InsertarFormulla($documento = '',$formula = '',$campos1 = '',$campos2 =''){
		$conn_erp = mysql_connect("localhost","root","") or	die("Could not connecta : ".mysql_error());
		mysql_select_db("jvmcompa_posweb_1",$conn_erp) or die("Could not select database <b>jvmcompa_posweb_1</b>");
	...
	*/
	...
	public function ConsultarFormula($documento = '',$formula = '',$tipo = ''){
		$conn_erp = mysql_connect("localhost","root","") or die("Could not connect : ".mysql_error());
		mysql_select_db("jvmcompa_posweb_1",$conn_erp) or die("Could not select database <b>jvmcompa_posweb_1</b>");
		if (!$documento) {
		...
		$SQL_5 = 'SELECT	id	FROM pos_pacientes WHERE account_number =  '.$documento;
	
		$result_5 = mysql_query($SQL_5,$conn_erp);
		$r_SQL_5 = mysql_fetch_assoc($result_5);
		$documento_2 =  $r_SQL_5['id'];
		...
		switch ($tipo) {
			case 'id':
				{
					$VALING = 'WHERE ps.customer_id="'.$documento_2.'" AND ps.formula="'.$formula.'"';
				}
				break;
			default:
				{
